...
var { typeForPodFile, hasPodNamespace } = require('../utils/pods-support');
  ...
  populateName: function() {
    ...
    if (!arePodsNamespaced) {
      if (podType) {
        ...
        // console.log(podType, typeFolder, fileName, '----',  this.sourceRelativePath, strippedRelativePath);
      }

      // for other files (adapters, serializers, helpers, initializers)
      if (type === undefined){
        pathRootRegex = new RegExp('(app\/)?');
        type = pathParts[1];
        fileName = pathParts[pathParts.length - 1];
        ...
        console.log('undefined type', fileName, type, typeFolder, this.sourceRelativePath, strippedRelativePath)
    ...
    // console.log(fileName, this.sourceRelativePath, strippedRelativePath, type, typeFolder);



    // var pathRootRegex = new RegExp('(app\/)?' + this.podModulePrefix + '\/(components\/)?');
